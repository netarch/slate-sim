...
def argparse_add_argument(parser):
    ...
    parser.add_argument('--NUM_REQUEST', type=list_of_ints, required=True)
...
if __name__ == "__main__":
    ...
    if model.Status == GRB.INFEASIBLE:
        ...
        solve_runtime = (solve_end_time - solve_start_time)*100
        constraint_setup_time = (constraint_setup_end_time - constraint_setup_start_time)*100
        ...
        print("** model.optimize() runtime: {} ms".format(round(solve_runtime, 2)))
        print("** constraint_setup_time runtime: {} ms".format(round(constraint_setup_time, 2)))
    ...
    print("App, NUM_CLUSTER, depth, fan_out_degree, no_child_constant, total_num_svc, total_num_remote_routing, optimizer_runtime, solve_runtime")
    ...
    now =datetime .datetime.now()
    g_.render(OUTPUT_DIR + now.strftime("%Y%m%d_%H:%M:%S") + '_call_graph', view = True) # output: call_graph.pdf
    g_
